...
def Train(trainset, train_labels):
    prior_probability = np.zeros(class_num)  # 先验概率P(y=ck)
    conditional_probability = np.zeros((class_num, feature_len, 2))  # 条件概率P(x=xl|y=ck)
    ...
    #prior_probability[label] = prior_probability[label]/len(test_labels) + 1.0#拉普拉斯平滑
    ...
    for ck in range(class_num):
        sum_ck = prior_probability[ck]
        prior_probability[ck] = prior_probability[ck]/len(train_labels)+1.0#拉普拉斯系数，防止概率为0
        for aj in range(feature_len):
            conditional_probability[ck][aj][0] = conditional_probability[ck][aj][0]+1.0/sum_ck+2.0
            conditional_probability[ck][aj][1] = conditional_probability[ck][aj][1]+1.0/sum_ck+2.0
