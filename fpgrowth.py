...
def createTree(dataSet, minSup=1):
    ...
    for trans in dataSet:  #第一次遍历：统计各个数据的频繁度
        for item in trans:
            """python3 dict.get(key, default=None),值不存在，返回默认的方法"""
...
def updateTree(items, inTree, headerTable, count):
    ...
    if len(items) > 1:  #仍有未分配完的树，迭代，list切片切片的三个参数分别代表开始索引、结束索引和步长值
...
def updateHeader(nodeToTest, targetNode):
    """
    :param nodeToTest: 原子节点
    :param targetNode: 新子节点
    :return: 新的FP树
    """
...
def createInitSet(dataSet):
    """
    :param:保存着源数据的列表，每一个元素是一个样本 
    :return:返回包含所有样本的字典，每一个元素对应的值是1，用于在生成头表的时候计数
    """
    retDict = {}
...
def findPrefixPath(basePat, treeNode):  #参数：指针（树名），节点；
    condPats = {}
    while treeNode != None:
        prefixPath = []
...
if __name__ == '__main__':
    with open('kosarak.txt') as DataFile:
        parseDat = [line.split() for line in DataFile.readlines()]
